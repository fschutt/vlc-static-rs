...
impl Media {
    ...
    pub fn new_location(instance: &Instance, mrl: &str) -> Option<Media> {
        ...
        unsafe{
            let p = (sys::get_vlc_dll().libvlc_media_new_location)(instance.ptr, cstr.as_ptr());
            if p.is_null() {
                return None;
            }
    ...
    pub fn new_path<T: AsRef<Path>>(instance: &Instance, path: T) -> Option<Media> {
        ...
        unsafe{
            let p = (sys::get_vlc_dll().libvlc_media_new_path)(instance.ptr, cstr.as_ptr());
            if p.is_null() {
                return None;
            }
    ...
    pub fn new_fd(instance: &Instance, fd: i32) -> Option<Media> {
        unsafe{
            let p = (sys::get_vlc_dll().libvlc_media_new_fd)(instance.ptr, fd);
            if p.is_null() {
                return None;
            }
    ...
    pub fn mrl(&self) -> Option<String> {
        unsafe{
            let p_str = (sys::get_vlc_dll().libvlc_media_get_mrl)(self.ptr);
            let s = from_cstr(p_str);
            (sys::get_vlc_dll().libvlc_free)(p_str as *mut ::libc::c_void);
            s
        }
    }
    ...
    pub fn event_manager<'a>(&'a self) -> EventManager<'a> {
        unsafe{
            let p = (sys::get_vlc_dll().libvlc_media_event_manager)(self.ptr);
            assert!(!p.is_null());
    ...
    pub fn get_meta(&self, meta: Meta) -> Option<String> {
        unsafe{
            let p_str = (sys::get_vlc_dll().libvlc_media_get_meta)(self.ptr, meta);
            let s = from_cstr(p_str);
            (sys::get_vlc_dll().libvlc_free)(p_str as *mut ::libc::c_void);
            s
        }
    ...
    pub fn set_meta(&self, meta: Meta, value: &str) {
        unsafe{
            (sys::get_vlc_dll().libvlc_media_set_meta)(self.ptr, meta, to_cstr(value).as_ptr());
        }
    }

    /// Save the meta previously set.
    pub fn save_meta(&self) -> bool {
        if unsafe{ (sys::get_vlc_dll().libvlc_media_save_meta)(self.ptr) } == 0 { false }else{ true }
    }

    /// Get current state of media descriptor object.
    pub fn state(&self) -> State {
        unsafe{ (sys::get_vlc_dll().libvlc_media_get_state)(self.ptr) }
    }

    /// Get duration (in ms) of media descriptor object item.
    pub fn duration(&self) -> Option<i64> {
        let time = unsafe{
            (sys::get_vlc_dll().libvlc_media_get_duration)(self.ptr)
        };
    ...
    /// Parse a media.
    pub fn parse(&self) {
        unsafe{ (sys::get_vlc_dll().libvlc_media_parse)(self.ptr) };
    }

    /// Parse a media.
    pub fn parse_async(&self) {
        unsafe{ (sys::get_vlc_dll().libvlc_media_parse_async)(self.ptr) };
    }

    /// Get Parsed status for media descriptor object.
    pub fn is_parsed(&self) -> bool {
        if unsafe{ (sys::get_vlc_dll().libvlc_media_is_parsed)(self.ptr) } == 0 { false }else{ true }
    }

    pub fn tracks(&self) -> Option<Vec<MediaTrack>> {
        unsafe{
            let mut p_track: *mut *mut sys::libvlc_media_track_t = ::std::ptr::null_mut();
            let n = (sys::get_vlc_dll().libvlc_media_tracks_get)(self.ptr, &mut p_track);
            if n == 0 {
                return None;
            }
            ...
            (sys::get_vlc_dll().libvlc_media_tracks_release)(p_track, n);
            Some(track)
        }
    }
...
impl Drop for Media {
    fn drop(&mut self) {
        unsafe{ (sys::get_vlc_dll().libvlc_media_release)(self.ptr) };
    }
}
